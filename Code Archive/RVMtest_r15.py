...
# r16 - database file name updated 
...
def hold_test(t, er, s, pr):
    ...
    if io.input(S1.IN) == 1:
        ...
        c.execute("UPDATE overview SET Pressure_ref=?", (pr[0],))
        conn.commit()
        ...
        if io.input(S1.IN) == 1 and io.input(S2.IN) == 1:
            ...
            c.execute("UPDATE overview SET hold_test_Pass=?,Pressure_dif=?", ("Pass", bar_dif,))
            conn.commit()
...
try:
    ...
    if __name__ == '__main__':
        ...
        database = "/home/pi/RVM/varificationDatabase_files/%s.db" % test_ID
        ...
        while i <= i_max:
            ...
            c.execute("UPDATE testData SET cycle_end=? WHERE cycle_number=(SELECT MAX(cycle_number) FROM testData)",
                      (time_stamp,))
            c.execute("UPDATE overview SET cycles_complete=?,error_count=?",
                      (i, errorCount))
            conn.commit()
