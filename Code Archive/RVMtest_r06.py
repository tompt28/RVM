...
def holdTest(hold_duration):
    ...
    if io.input(17) == 1:
        ...
        if io.input(18) == 0:
            ...
            c.execute("UPDATE overview SET hold_test_Pass=?,cycle_test_Pass=?,notes=?,errorCount=?,end_date",("Fail","Fail","no pressure registered at output test failed",errorCount,timeStamp))
    ...
    if __name__ == '__main__':
        ...
        while i <= i_max:
            ...
            if testGo == False:
                ...
                c.execute("UPDATE testData SET error=?,notes=?",(errorCount,"input pressure lost- Test Paused"))
                ...
                c.execute("SELECT datetime('now','localtime')")
                timeStamp= c.fetchone()[0]
                c.execute("UPDATE testData SET error=?,notes=?",(errorCount,"no pressure registered at output- Test failed",))
                ...
                c.execute("UPDATE testData SET cycle_end=?,error=?",(timeStamp, errorCount))
