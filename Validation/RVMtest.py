...
def cycle_max():
    ...
    """
    y = - 1
    while y not in range(1, 10000000):
        try:
...
def hold_test(t, er, s, pr):
        ...
        while io.input(S0.IN) == 0:
            ...
            while noIn == False:
                ...
                if noIn[0].upper() == "N":
                    ...
                    c.execute("UPDATE Hold_test SET hold_test_Pass=?, Pressure_ref=?, hold_duration=?, Pressure_held=?, Pressure_dif=?, Error=? WHERE Test=(SELECT MAX(Test)FROM Hold_test)",
                        ("Fail", 0, 0, 0, 0, er))
                    c.execute("UPDATE Overview SET hold_test_Pass=? Error_count=? WHERE Test=(SELECT MAX(Test)FROM Overview)", ("Fail", er))
                    conn.commit()
    ...
    if io.input(S1.IN) == 1:
        ...
        c.execute("UPDATE Hold_test SET Pressure_ref=? WHERE Test=(SELECT MAX(Test)FROM Hold_test)", (pr[0],))
        conn.commit()
        ...
        if io.input(S1.IN) == 1 and io.input(S2.IN) == 1:
            ...
            if pr[1] / pr[0] <0.80 :
                er += 1
                print("More than 20 percent of the pressure has been lost, review the leak tightness of the assembly") 
                

                c.execute("SELECT datetime('now','localtime')")
                time_stamp = c.fetchone()[0]

                c.execute("UPDATE Hold_test SET Hold_test_Pass=?, Pressure_held=?, Pressure_dif=?, Error=? WHERE Test=(SELECT MAX(Test)FROM Hold_test)",
                    ("Fail", pressure_held, bar_dif, er))
                c.execute("UPDATE Overview SET Hold_test_Pass=?, Error_count=?, Notes=?, End_date=? WHERE Test=(SELECT MAX(Test)FROM Overview)", ("Fail", er, "leakage is outside acceptable range",time_stamp))
                conn.commit()
                ...
                c.execute("UPDATE Hold_test SET Hold_test_Pass=?, Pressure_held=?, Pressure_dif=? WHERE Test=(SELECT MAX(Test)FROM Hold_test)", ("Pass", pressure_held, bar_dif))
                c.execute("UPDATE Overview SET Hold_test_Pass=? WHERE Test=(SELECT MAX(Test)FROM Overview)", ("Pass",))
                conn.commit()
            ...
            c.execute("SELECT datetime('now','localtime')")
            time_stamp = c.fetchone()[0]
            c.execute("UPDATE Overview SET Hold_test_Pass=?, Notes=?, Error_count=?, End_date=? WHERE Test=(SELECT MAX(Test)FROM Overview)",
...
def Cycle_Check(er, major_error):
    '''
    Checks the input and output
    takes in:
    er = Current error count
    major_error = Current Major error count
    ...
    if not testGo:
        ...
        c.execute(
            ...
            (er, "input pressure lost - Test Paused"))
        c.execute("UPDATE Overview SET Error_count=?, Notes=? WHERE Test=(SELECT MAX(Test)FROM Overview)",
            (er, "input pressure lost- Test Paused"))
    ...
    if not outputOn:
        ...
        c.execute("UPDATE Overview SET Cycle_test_Pass=?, End_date=?, Error_count=?, Notes=?",
            ("Fail", time_stamp, major_error, "Test failed Output lost during cycle test"))
...
try:
    ...
    if Email == "Y":
        print("\nhi {},\nIf there is an error i'll try to reach you at {},\nThe hold test will run over {}s and will "
              "then perform {} cycles and 30 double failure Scenarios.".format(tester, receiver_email, hold_duration, i_max))
    else:
        print("\nhi {},\nYou have selected not to have email reporting.\nPlease check on the test periodically.\n"
              "The hold test will run over {}s and will then perform {} cycles and 30 double failure Scenarios.".format(tester, hold_duration, i_max))

    input("\nPress Enter to start the test")
    start = datetime.datetime.now()

    fileName = "/home/pi/RVM/Validation/Database/%s-%s-%s.sqlite" % (Sales_Order,Part_code,Serial)
    ...
    if __name__ == '__main__':
        database = "/home/pi/RVM/Validation/Database/%s-%s-%s.sqlite" % (Sales_Order,Part_code,Serial)
        ...
        while i <= i_max:
            ...
            for x in cyclelist:
                ...
                check = Cycle_Check(errorCount, major_error)
                if not check[0]:
                    #raise KeyboardInterrupt
                ...
                if not check[0]:
                    ...
                    if x.name == "v1":
                        ...
                        print("ERROR outside valve range")
        ...
        c.execute("UPDATE Overview SET Cycle_test_Pass=? WHERE Test=(SELECT MAX(Test)FROM Overview)", ("Pass",))
        conn.commit()
        # make sure all off!
        io.output(v0.out, 1)
        # De-energise all 4 coils+leds
            ...
            for k, x in Scenario_dict.items():
                ...
                if k == "1" or k == "4" or k == "6":
                    if outputOn:
                        ...
                        c.execute("UPDATE Overview SET Scenario_Test_Pass=?, Notes=?, End_date=? WHERE Test=(SELECT MAX(Test)FROM Overview)",
        ...
        c.execute("UPDATE Overview SET Scenario_Test_Pass=? WHERE Test=(SELECT MAX(Test)FROM Overview)",("Pass",))
        conn.commit()
        ...
        c.execute("UPDATE Overview SET End_date=? WHERE Test=(SELECT MAX(Test)FROM Overview)",(end,))
        conn.commit()
        conn.close()
